...
\title{\textbf{Detalhamento da Implementação: Perceptron Ecofuncional}\\\small Atualizado para v0.2.1 (vetor temporal de 30 features)}
...
\begin{itemize}
    \item \textbf{calculateDelta}: derivada discreta entre os dois últimos estados;
    \item \textbf{calculateAverage}: média móvel (janela configurável, uso padrão 3);
    \item \textbf{analyzeState}: classifica a trajetória (estável, recuperação, degradação, colapso).
\end{itemize}

\subsection{Feature Vector Temporal (30)}
...
\begin{enumerate}
    \item Carrega `data/training_data.csv` (10 atributos + \textit{FunctionalIntegrity}) com validação de colunas e cabeçalho.
    \item Treina supervisionadamente via `PerceptronTrainingService`, gerando features temporais (30).
    \item Carrega `data/trajectory_data.csv` (10 atributos) e executa inferência incremental para cada passo.
    \item Persiste o log de inferência em JSON (`outputs/inference_results.json`) para visualização.
\end{enumerate}
...
O serviço de inferência consome o vetor de 30 entradas (estado + delta + média móvel) e usa a classificação da trajetória (estável/recuperação/degradação/colapso) para atribuir a métrica de \textit{RecoveryCapacity}.
...
\section{Fase 4: Engenharia de Dados e Visualização}
...
\subsection{Pipeline de Carga (Data Loader)}
...
\begin{itemize}
    \item \textbf{Training Adapter}: Converte linhas CSV em `EcofunctionalExperiment`.
    \item \textbf{Trajectory Adapter}: Converte séries temporais em `EcofunctionalTrajectory`.
\end{itemize}

\subsection{Visualização de Histerese}
Um script auxiliar em Python (`scripts/plot_trajectory.py`) consome os logs de inferência (JSON) gerados pelo sistema C++. Ele plota a \textit{Integridade Funcional} ao longo do tempo e ajuda a visualizar visualmente os fenômenos de histerese e inércia ecológica capturados pela lógica de domínio.
