...
\begin{enumerate}
    \item Carrega `training_data.csv` (10 atributos + alvo) com validação de colunas e cabeçalho.
    \item Treina supervisionadamente via `PerceptronTrainingService`, gerando features temporais (30).
    \item Carrega `trajectory_data.csv` (10 atributos) e executa inferência incremental para cada passo.
    \item Persiste o log de inferência em JSON (`inference_results.json`) para visualização.
\end{enumerate}
...
Um script auxiliar em Python (`plot_trajectory.py`) consome os logs de inferência (JSON) gerados pelo sistema C++. Ele plota a \textit{Integridade Funcional} ao longo do tempo e ajuda a visualizar visualmente os fenômenos de histerese e inércia ecológica capturados pela lógica de domínio.
